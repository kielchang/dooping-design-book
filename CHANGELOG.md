...
## 2026-09-11（純文件進版，版號未動）

### 治理：計畫與決定移到 PMIS，repo 只留規則與架構描述（版號不動）

1. **改了什麼**：
   - 移除 `docs/adr/`（13 則＋README）、文件站的「決策紀錄」章與 `book/scripts/sync-adr.mjs`；
     `AGENTS.md`、`llms.txt`、首頁的 ADR 入口一併拿掉。
   - 文件裡 29 個指向 ADR 的連結改成**一句話理由寫回原地**——規則旁邊就有理由，不必跳頁。
   - 流程改寫（五份複本同步）：RFC 接受後在維護方的 PMIS 開有到期日的計畫，結案時成立了難回頭的決定才記 ADR；
     符合性台帳的順序改成「**先在宿主做、台帳記自製，三次法則過了再提回上游**」（原本的「未合併不得先實作」會讓每個宿主卡在同一個提案上）。
   - `CLAUDE.md` 新增「計畫、規則、決定分三軌」與「換電腦接續」兩節；agent 不得在 repo 開 ADR 或提案檔。
2. **我需要做什麼**：不需要。元件與 token 沒動；元件註解裡的 `ADR-00xx` 編號保留，指向 PMIS 同號。
3. **為什麼改**：ADR 被拿來做開發規劃，結果太瑣碎、規劃好就變限制、而且沒有驗證——13 則只有 6 則被守衛點名，
   有一則改了六次仍是「提議中」。真正防止再犯的是會失敗的守衛，所以 repo 只留規則、架構描述與使用說明；
   計畫（有到期日）與決定（四段式）記在 PMIS。舊 ADR 全文仍在此 commit 之前的 git 歷史。
...
**改了什麼**：ci.yml 與 deploy.yml 給 Chromium 加 `actions/cache`
（key 綁 package-lock；命中時只補系統相依）。
**我需要做什麼**：不用。
**為什麼改**：實測 Chromium 下載 622 秒、渲染守衛本身 15 秒——
而 ci 每次 dev push／PR 都在付這 10 分鐘。快取後回到約半分鐘。
