...
| 座標 | 什麼時候動 | 誰動 | 回答什麼問題 |
| --- | --- | --- | --- |
| `@dooping/tokens` 的 npm 版號 | token 內容變更 | 人（bump 三處版號） | 取用端要鎖哪一版 |
| `packages/react` 的版號（＝registry 戳記） | 元件變更 | 人（bump ＋ 重產 registry） | 我抄走的是哪一版 |
| **`release-YYYY-MM-DD` tag** | 每次進版 | **部署自動蓋** | `main` 在那個時間點的完整狀態 |

進版 tag 用日期而不是語意版號，因為一次進版不一定伴隨 token 或元件的版本變動
（例如純文件的進版）。

**`release-*` 不需要手動打。** `main` 部署成功後，`deploy.yml` 會自動蓋上
annotated tag（訊息帶 commit 與部署 run 的連結；同一天多次進版自動加 `-2`、`-3`；
重跑部署不會重複打）。tag 是部署的**紀錄**，不是閘門——真正的閘門是
「要不要把 `dev` 併進 `main`」這個決定。
...
自動蓋的 `release-*` 不會誤觸發發佈：它不符合 `tokens-v*` 的比對模式，
