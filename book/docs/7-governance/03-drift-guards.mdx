...
`@dooping/tokens` 的兩個 Tailwind 出口都**清空**預設色盤，套用之後 Tailwind 的預設色盤整個消失：

```css title="Tailwind v4：@dooping/tokens/tailwind.css 裡的這一行"
...
```js title="Tailwind v3：preset 覆蓋（而非 extend）theme.colors"
module.exports = {
...
以下八支都可以直接抄，但**不是每一支都在這個 repo 裡跑**——分清楚比較有用：
...
第 6 與第 8 支是寫給**有獨立文件站的宿主專案**的，本 repo 的架構剛好不需要。
「不適用」不等於「不重要」——它們擋的漂移在那種架構下一樣真實。

規則的標示只有五種，全書統一：**✅ 在跑**（有會失敗的守衛）、**部分**（守衛只蓋到一部分）、**建議**（沒有守衛，取用端自行決定）、
**不適用**、**人工：由誰、何時**（沒有守衛，明說誰在什麼時點檢查）。沒有守衛的規則不能寫成「✅」——
它就是建議或人工，寫清楚比假裝有守衛好。本 repo 全部守衛各管什麼、不管什麼，見
[架構](/governance/architecture)的守衛台帳。
...
// 掃描 packages/、book/docs、.github、registry
expect(hits).toEqual([]);
```
...
更要緊的是取用端：他們不一定套我們的 preset（[token 是唯一硬相依](/start/three-ways)），
...
**特別要盯琥珀**：`--edit` 是保留色（見[提醒色辭典](/foundations/alert-colors)）。一旦有人用原始色階寫了一個
...
Tailwind v4 宿主沒有 `tailwind.config.js`：把第一行換成讀全域 CSS——
`@theme` 區塊裡的 `var(--…)` 就是「樣式設定實際引用的變數」，其餘照舊。
...
[零截圖文件示意](/patterns/zero-screenshot-docs#互動示意什麼時候該嵌-storybook)
的第一個嵌入一起加的）。實作時有兩件事是量出來才知道的：

**一、id 的推導必須對著真值驗一次。** 這支守衛的核心是「用 `meta.title` ＋ export 名
重算 Storybook 的 id」，而重算就是**第二份真相**——推導規則與 Storybook 稍有出入，
...
但圍籬裡的範例**要驗**：那是讀者會複製走的東西，範例裡的 id 死掉一樣是壞的，
只是晚一步才被發現。
...
還有一個小陷阱：好幾個部署寫入的是**同一個發佈分支的不同子目錄**（本書是正式站、`preview/`、`staging/` 三段）。
沒有協調就會互相覆蓋——症狀是「明明部署成功了，但站上還是舊的」。
直覺解法是全部共用一個 concurrency group，但 GitHub 一個 group 只留一個**等待中**的 run：
連推兩次，排在後面的就把等待中的另一段部署取消掉。本書的做法是**每段一組**，
部署腳本只准動自己的目錄，push 被拒時重抓分支、重套自己的目錄、再推（`scripts/deploy-gh-pages.sh`）。
