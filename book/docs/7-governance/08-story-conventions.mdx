...
:::note story 的 title、export 名與 storySort 分類不翻譯
它們是三支守衛的**鍵**：`tests/doc-hooks.test.ts` 用 title 推導文件裡 `<StoryFrame id>` 的合法集合，
`tests/story-sort.test.ts` 逐字比對 `.storybook/preview.tsx` 的分類字串，`scripts/verify-storybook.mjs` 的 play 用中文可及名稱查元素。
翻了 title，文件的嵌入範例會安靜地變成空白 iframe。元件內建文案的預設值同理——英文宿主走 `labels` prop 覆寫，不動預設值。
:::
...
story 並列的是**長相**，play function 驗的是**行為**——焦點去哪裡、鍵盤按了會怎樣、
aria 屬性有沒有連動。判準很簡單：**文件頁寫了行為規範的元件，就要有一支 play function
把那條規範變成可執行的斷言**。對話框寫了「Esc 關閉、焦點回到觸發鈕」，play 就按一次
Esc 驗一次；欄位錯誤寫了「aria-describedby 指向錯誤訊息」，play 就查那條連線。

寫法慣例（現有的 play 都照這個模式）：
...
<Rules>
  <Do>
    <p>portal 元件（Dialog、Select、Toast、篩選面板）的斷言要查
    <code>canvasElement.ownerDocument.body</code>——浮層掛在 body 直下，
    canvas 子樹裡找不到它。</p>
  </Do>
  <Do>
    <p>查詢一律走 <code>getByRole</code> 帶可及名稱。這讓 play 順便當了半個
    無障礙測試：role 或名稱斷了，查詢就失敗。</p>
  </Do>
  <Dont>
    <p>不要在 play 結尾留下開著的浮層或改掉的狀態——後面的視覺掃描拍的是
    play 跑完的畫面，殘留狀態會讓兩支守衛互相干擾。</p>
  </Dont>
</Rules>

### 對輸入框設值：用 setInputValue，不用 userEvent.type

`userEvent.type`／`clear`／`paste` 的文字插入在部分 Chromium 環境
（實測 Electron 內嵌瀏覽器）會經元素實例的 value descriptor 寫值，
同步更新 React 的 value tracker——接著派發的 input 事件被 React 判定
「值沒變」而吞掉 onChange。結果是**受控輸入的狀態不更新、play 在無頭 CI
綠、真瀏覽器紅**：preview 評估期間任何人用瀏覽器開 Storybook 都會看到假性失敗。

打字一律改用 `demo/play.ts` 的 `setInputValue`（prototype setter 繞過
tracker，所有環境行為一致）；它是**一次設整段值**，要驗中間狀態就分次呼叫、
每次給完整目標值。鍵盤與點擊不受影響，照常用 `userEvent.keyboard`／`click`。
這條有守衛：`tests/play-conventions.test.ts` 掃描 stories，違規直接紅。

### Play 的性質不能隨視窗寬度改變

會依 media query 切換行為的元件（外殼、抽屜），story 要把 query 釘死：
桌面版釘 `(max-width: 0px)`（永不成立＝強制桌面）、行動版釘
`(min-width: 0px)`（恆成立＝強制行動）。不釘的話，觀看者的視窗寬度會改變
play 驗的是哪一套行為——窄視窗下桌面斷言找不到地標，一樣是「CI 綠、真瀏覽器紅」。
